...
async function cryptoPriceFromServer() {
  const currentBtcPrice = await axios
    .get(`${baseURL}/price?fsym=BTC&tsyms=USD`)
    .then(res => res.data);

  if (currentBtcPrice.USD) {
    return Number(currentBtcPrice.USD);
  }
}
...
module.exports = {
  ...
  convertToNGN,
  calculate: async function(type, margin, exchangeRate) {
